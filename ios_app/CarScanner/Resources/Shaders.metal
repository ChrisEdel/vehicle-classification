/*
 ...
 Abstract:
 The car scanner app's shaders.
 */

#include <metal_stdlib>
#include <simd/simd.h>
#import "../Models/ShaderTypes.h"
...
vertex void unprojectVertex(uint vertexID [[vertex_id]],
    ...
    if (depth > uniforms.distanceThreshold) {
        particleUniforms[currentPointIndex].confidence = -1;
    } else {
        particleUniforms[currentPointIndex].confidence = confidence;
    }
}

///  Vertex shader that takes in a vertexID and returns `RGBVertexOut`.
...
///  Fragment shader that takes in a `RGBVertexOut` and returns the corresponding color.
...
///  Vertex shader that gets the point data from a given vertexID and projects the point.
...
///  Fragment shader that takes in a `ParticleVertexOut` and returns the corresponding color.
